...
echo "$(kubectl -n gloo-system get service gateway-proxy -o jsonpath='{.status.loadBalancer.ingress[0].ip}') argocd-local.glootest.com api-local.glootest.com portal-local.glootest.com"
echo "EOF"
echo
